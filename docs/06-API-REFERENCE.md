...
| GET | `/api/sessions/{id}` | — | `{ session, metrics, events }` — metrics include parsed `payload` |
...
| POST | `/api/listening/{id}/end` | — | Builds summary via `listening_summary` |
...
| GET | `/api/exercises` | — | Exercises, recommended, streak, mission, plan, hard_words, … |
...
Origins from `CORS_ORIGINS` (comma-separated). If empty string after parse, middleware falls back to `["*"]` with credentials allowed (see `main.py`).
